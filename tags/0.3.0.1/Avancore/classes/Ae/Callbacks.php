...
class Ae_Callbacks {
    ...
    function getInstance() {
        if (!self::$instance) self::$instance = new Ae_Callbacks();
        return self::$instance;
    }
    
    function setInstance(Ae_Callbacks $instance) {
                ...
                if (is_array($callback)) {
                    if (is_array($cb) && $cb[1] == $callback[1]) {
                        if (is_object($callback[0]) && is_object($cb[0])) {
                            if (Ae_Util::sameObject($callback[0], $cb[0])) {
                                $match = true;
                            }
    ...
    function call($callbackId, $params) {
        if (!isset($this) || !is_a($this, 'Ae_Callbacks')) $cb = & Ae_Callbacks::getInstance();
            else $cb = & $this;
