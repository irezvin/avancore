...
class Ae_Tree_NestedSetsMapper extends Ae_Model_Mapper implements Ae_I_Tree_Mapper_NestedSets {
    ...
    /**
     * Prototype of Ae_Sql_NestedSets
     */
    ...
    function getNestedSets() {
        if ($this->_nestedSets === false) {
            ...
            if (!isset($proto['blocker'])) $proto['blocker'] = new Ae_Sql_Blocker();
            if (!isset($proto['db'])) $proto['db'] = new Ae_Sql_Db_Ae($this->database);
        	...
        	$this->_nestedSets = new Ae_Sql_NestedSets($proto);
        }
    ...
    function loadNodes(array $ids) {
        ...
        foreach ($nodes as $id => $node) {
            $objNode = new Ae_Tree_NestedSetsImpl(array(
                'nodeData' => $node,
                'mapper' => $this, 
            ));
            $res[$id] = $objNode;
    ...
    function getNsRelation() {
        ...
        if ($this->_nsRelation === false) {
            $this->_nsRelation = new Ae_Model_Relation(array(
    ...
    function getAllChildrenCountRelation() {
        if ($this->_allChildrenCountRelation === false) {
            $ns = $this->getNestedSets();
            $this->_allChildrenCountRelation = new Ae_Model_Relation(array(
                'srcTableName' => $this->tableName,
                'destTableName' => new Ae_Sql_Expression("(
    ...
    function createTreeProvider() {
    	return new Ae_Tree_Provider($this);
    }
        ...
        if (strlen($pId)) {
            ...
            foreach ($ords as $ord) {
                ...
                if ($ord['id'] == $modelObject->id) {
                    $lbl .= ' '.(new Ae_Lang_String('model_ordering_current'));
                    $foundMyself = true;
                }
                $res[$ord['ordering']] = $lbl;   
            }
        } else $ords = array();
        if (!count($ords)) $res[' 0'] = new Ae_Lang_String('model_ordering_only');
        elseif (!$foundMyself) {
            $res[' '.($ord['ordering'] + 1)] = new Ae_Lang_String('model_ordering_last');
        }
        return $res;
    }
    ...
    function createTreeImpl(Ae_Model_Object $modelObject) {
        return new Ae_Tree_NestedSetsImpl(array(
    ...
    function fixTree($dontApply = false, $extraColumns = '') {
		...
		if (!$dontApply) {
    		...
    		$s = new Ae_Sql_Statement("UPDATE {$treeTableName} SET treeId = {{bak}} WHERE treeId = {{curr}}", array(
    			'curr' => $this->nsTreeId, 'bak' => $treeId2
    		));
    		$sql->query($s);
    		$s = new Ae_Sql_Statement("UPDATE {$treeTableName} SET treeId = {{curr}} WHERE treeId = {{temp}}", array(
	...
	function saveTreeArr($items, Ae_Sql_NestedSets $ns, $rootId, $oldRootId) {
