...
class Ae_Lang_ResourceProvider_Dir extends Ae_Autoparams implements Ae_I_Lang_ResourceProvider {
	...
	static function autoRegister($dirNamesString = false) {
        ...
        $lrp = array();
		$lr = Ae_Lang_Resource::getInstance();
		if ($dirNamesString !== false) {
			$dirNames = explode(PATH_SEPARATOR, $dirNamesString);
			$lrp = $lr->getProviders();
			foreach ($dirNames as $dirName) $lrp[] = new Ae_Lang_ResourceProvider_Dir(array('dirName' => $dirName));
		}
		if ($lrp) {
