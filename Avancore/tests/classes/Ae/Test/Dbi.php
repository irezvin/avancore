...
class Ae_Test_Dbi extends Ae_Test_Base {

	// --------------------------------+ supplementary functions +-----------------------------------
	
	/**
	 * @param Ae_Sql_Dbi_Database $dbiDatabase
	 * @return unknown_type
	 */
	...
	function & createDbiDatabaseWithMySql5Inspector() {
		$sqlDb = new Ae_Sql_Db_Ae(Ae_Dispatcher::getInstance()->database);
	...
	function & createDbiDatabaseWithDefaultInspector() {
		$sqlDb = new Ae_Sql_Db_Ae(Ae_Dispatcher::getInstance()->database);
		$inspector = new Ae_Sql_Dbi_Inspector($sqlDb, $this->getDbName());
		$dbiDb = new Ae_Sql_Dbi_Database($inspector, $this->getDbName(), $this->getTablePrefix());
		return $dbiDb;
	}
	...
	function & createDbiDatabaseWithLegacyInspector() {
		...
		$lcl = new Ae_Util_LegacyAvancoreLoader(array('avancorePath' => dirname(__FILE__).'/../../../legacy'));
		$lcl->load();
		$inspector = new Ae_Sql_Dbi_Inspector_Legacy('ac_'); 
		$dbiDb = new Ae_Sql_Dbi_Database($inspector, $this->getDbName(), $this->getTablePrefix());
		return $dbiDb;
	}
