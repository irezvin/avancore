 ...
 */
class Ae_Test_RefChecker extends Ae_Test_Base {
	...
	function & getDbi() {
		$sqlDb = & $this->getSqlDb();
		$inspector = new Ae_Sql_Dbi_Inspector_MySql5($sqlDb, $this->getDbName());
		$dbiDb = new Ae_Sql_Dbi_Database($inspector, $this->getDbName(), $this->getTablePrefix());
		return $dbiDb;
	}
	 ...
	 */
	function & getSqlDb() {
		if ($this->_sqlDb === false) $this->_sqlDb = new Ae_Sql_Db_Ae(Ae_Dispatcher::getInstance()->database);
		return $this->_sqlDb;
	}

	/**
	 * @return Ae_Sql_RefChecker
	 */
	function & createRefChecker() {
		$dbi = & $this->getDbi();
		$sdb = & $this->getSqlDb();
		$rc = new Ae_Sql_RefChecker(array(
			'db' => & $sdb,
			'schema' => & $dbi
		));
		return $rc;
	...
	function testRefChecker() {
		...
		$this->assertEqual($a = $rc->listStatColumns($select3), $b = array(
			($k = '#__people') => array(
				(($m = 1) - 1) => array(
					($l = AE_RC_NUM_OUTGOING) => "rel_{$peopleName}_{$m}_{$l}",
					($l = AE_RC_NUM_NO_REF) => "rel_{$peopleName}_{$m}_{$l}",
					($l = AE_RC_NUM_CORRECT) => "rel_{$peopleName}_{$m}_{$l}",
					($l = AE_RC_NUM_MISSING) => "rel_{$peopleName}_{$m}_{$l}",
				),
				(($m = 2) - 1) => array(
					($l = AE_RC_NUM_OUTGOING) => "rel_{$peopleName}_{$m}_{$l}",
					($l = AE_RC_NUM_NO_REF) => "rel_{$peopleName}_{$m}_{$l}",
			...
			($k = '#__relation_types') => array(
				0 => array(
					($l = AE_RC_NUM_OUTGOING) => "rel_{$relationTypesName}_{$l}",
					($l = AE_RC_NUM_NO_REF) => "rel_{$relationTypesName}_{$l}",
					($l = AE_RC_NUM_CORRECT) => "rel_{$relationTypesName}_{$l}",
					($l = AE_RC_NUM_MISSING) => "rel_{$relationTypesName}_{$l}",
				),
			),
