<?php
    ...
    class RESIZEIMAGE
    {
        ...
        function resize_limitwh($imgwidth,$imgheight,$newfile=NULL)
        {
            ...
            //if ($width > $height) $image_per = ($imgwidth / $width) * 100;
            //    else $image_per = ($imgheight / $height) * 100;
            
            $image_per = ($imgwidth * 100) / $width;

            if(floor(($height * $image_per)/100) > $imgheight)
                $image_per = ($imgheight * 100) / $height;
            

            // hack
            $this->newWidth = $image_per/100*$width;
            $this->newHeight = $image_per/100*$height; 
