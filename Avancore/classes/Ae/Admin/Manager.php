...
class Ae_Admin_Manager extends Ae_Legacy_Controller {
     ...
     * @access protected 
     * @var Ae_Admin_Datalink
     */
    var $_datalink = false;
    
    /**
     * @var Ae_Admin_Template
     */
    var $_template = false;

    var $_templateClass = 'Ae_Admin_Template';
     ...
     * @var array of Ae_Manager_Feature
     */
     ...
     * TODO: connect method implementation to the user-supplied connector class that should be responsible for concrete Ae_Admin_Manager behavior
     * (perhaps Ae_Admin_Datalink) 
     *
    ...
    /**
     * @param Ae_Model_Datalink $datalink
     */
    function setDatalink(& $datalink) {
        if (is_object($datalink)) {
            if (!is_a($datalink, 'Ae_Admin_Datalink')) 
                trigger_error ("\$datalink should be Ae_Admin_Datalink instance", E_USER_ERROR);
    ...
    function getForm() {
        if ($this->_form === false) {
            if ($record = & $this->getRecord()) {
                ...
                if (!is_a($this->_form, 'Ae_Form')) trigger_error ("'{$class}' is not descendant of Ae_Form", E_USER_ERROR);
    ...
    function getFilterForm() {
        if ($this->_filterForm === false) {
            ...
            if (isset($formConfig['controls']) && is_array($formConfig['controls']) && count($formConfig['controls'])) {
                ...
                if (!is_a($this->_filterForm, 'Ae_Form')) trigger_error ("'{$class}' is not descendant of Ae_Form", E_USER_ERROR);
                foreach ($this->listFeatures() as $f) {
                    $feat = & $this->getFeature($f);
...
/**
    ...
    function getRecordPrototype() {
        if ($this->_recordPrototype === false) {
            ...
            if ($this->onlyRecord) { 
                if (is_a($this->onlyRecord, 'Ae_Model_Data')) $this->_recordPrototype = & $this->onlyRecord;
                else trigger_error("$onlyRecord must be descendant of Ae_Model_Data", E_USER_ERROR);
            } else {
        ...
        if ($this->_featureObjects === false) {
            ...
            if ($this->mapperClass) {
                ...
                if (is_array($info->adminFeatures)) {
                    Ae_Util::ms($this->featureSettings, $info->adminFeatures);
                }
            }
                    ...
                    if (is_array($this->prohibitSubsystems)) foreach ($this->prohibitSubsystems as $subsystemPrefix) {
                        foreach (array_keys($smc) as $k) {
                            if (isset($smc[$k]['mapperClass']) && !strncmp($smc[$k]['mapperClass'], $subsystemPrefix, strlen($subsystemPrefix) ))
                                ...
                                Ae_Util::ms($smc[$k]['prohibitSubsystems'], $this->prohibitSubsystems);
                            }
                        }
                    }
                    Ae_Util::ms($this->_subManagers, $smc);   
                }
            }
...
//            'childWrapperTemplateClass' => 'Ae_Form_Control_Template_Basic',
//            'childWrapperTemplatePart' => 'divWrapper',
    ...
    function _getPreloadRelations() {
        if ($this->_preloadRelations === false) {
            ...
            foreach ($this->listFeatures() as $f) {
                $feat = & $this->getFeature($f);
                Ae_Util::ms($this->_preloadRelations, $feat->getPreloadRelations());
            }
        }
    }
        ...
        if ($this->_sqlFilter === false) {
            ...
            if (!count($allFilters)) $this->_sqlFilter = null;
                ...
                if (0 && (count($allFilters) == 1)) {
                    $filterSettings = Ae_Util::array_values(array_slice($allFilters, 0, 1));
                    $filterSettings = $allFilters[0];
                } else {
                    $filterSettings = array(
                        'class' => 'Ae_Sql_Filter_Multiple',
                        'filters' => $allFilters, 
                    );
                }
        ...
        if ($this->_sqlOrder === false) {
            ...
            if (!count($allOrders)) $this->_sqlOrder = null;
                ...
                if (0 && (count($allOrders) == 1)) {
                    $orderSettings = Ae_Util::array_values(array_slice($allOrders, 0, 1));
                    $orderSettings = $orderSettings[0];
                } else {
                    $orderSettings = array(
                        'class' => 'Ae_Sql_Order_Multiple',
                        'orders' => $allOrders, 
                        'applied' => 1,
                    );
                }
    ...
    function & _getSqlSelect() {
        if ($this->_sqlSelect === false) {
            ...
            foreach ($this->listFeatures() as $i) {
                ...
                Ae_Util::ms($options, $feat->getSqlSelectSettings());
            }
            ...
            if ($f || $o || $options) {
                ...
                $this->_sqlSelect = new Ae_Sql_Select($sqlDb, $options);
    ...
    function _getRecordClass() {
        if ($this->_recordClass === false) {
            if ($this->onlyRecord) {
                if (is_a($this->onlyRecord, 'Ae_Model_Data')) $this->_recordClass = get_class($this->onlyRecord);
                    else trigger_error ('$onlyRecord property must be set to a valid Ae_Model_Data instance', E_USER_ERROR);
            } else {
                 $mapper = & $this->getMapper();
