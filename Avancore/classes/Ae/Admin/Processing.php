...
class Ae_Admin_Processing extends Ae_Legacy_Controller {
    ...
    function doInitProperties($options = array()) {
        ...
        if (isset($options['datalink'])) {
            trigger_error ("Cannot set datalink in the constructor - call setDatalink() after Ae_Admin_Processing instantiation", E_USER_ERROR);
        }
    ...
    function setDatalink(& $datalink) {
        ...
        if ($datalink) {
            if (!is_a($datalink, 'Ae_Admin_Datalink')) 
                trigger_error("\$datalink must be instance of Ae_Admin_Datalink", E_USER_ERROR);
            else {
    ...
    /**
     * @param Ae_Admin_ReportEntry $reportEntry
     */
    function setReport (& $reportEntry) {
        if ($reportEntry && !is_a($reportEntry, 'Ae_Admin_ReportEntry'))
            trigger_error ("\$reportEntry should be instance of Ae_Admin_ReportEntry");
        
        $this->_report = & $reportEntry;
    }
    
    /**
     * @return Ae_Admin_ReportEntry
     */
    function getReport() {
        return $this->_report;    
    }
    ...
    function reportRecord(& $record, $description, $type = 'message', $dateTime = false, $isAvailable = true) {
        if (is_a($record, 'Ae_Model_Object') && ($m = & $record->getMapper()) && ($tf = $m->getTitleFieldName())) $title = $record->getField($tf);
    ...
    function reportRecordErrors(& $record, $forceCheck = false) {
        if ($forceCheck || $record->isChecked()) {
            if ($errs = $record->getErrors()) {
                $this->reportRecord($record, nl2br(htmlspecialchars(Ae_Util::implode_r("\n", $errs))), 'error');
            }
        }
    }
