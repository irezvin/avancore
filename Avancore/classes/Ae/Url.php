...
class Ae_Url {
    ...
    /**
     * @param string strUrl Ae_Url string to populate scheme, host, path etc...
     */
    function Ae_Url($strUrl = false) {
        if ($strUrl !== false) {
            if ($strUrl instanceof Ae_Url) {
    ...
    function getQueryString() {
        return Ae_Url::array2queryString($this->query);
    }
     ...
     *
     * <code>
     * 		$u = new Ae_Url("http://example.com/foo/bar/baz");
     * 
     * 		echo $u->path; 
     * 		// 		"/foo/bar/baz"
     * 
...
//                    else $rUri .= $withQuery && $this->query ? '?'.(Ae_Url::array2queryString($this->query)) : '';
            }
            ...
            if (!strlen($this->path) && !strlen($this->pathInfo) && (($withQuery && $this->query) || strlen($this->fragment)) && substr($uri, -1) !== '/') $uri .= '/';
            //$uri .= $withQuery && strlen($this->query) ? '?'.(Ae_Url::array2queryString($this->query)) : '';
            if ($withQuery && $this->query) {
    ...
    function array2queryString($arr_request, $var_name = '', $separator='&') {
        ...
        if (is_array($arr_request)) {
            foreach ($arr_request as $key => $value) {
                if (is_object($value) && is_a($value, 'Ae_Url')) $value = $value->toString();
                if (is_array($value)) {
                    if ($var_name) {
                        $ret .= Ae_Url::array2queryString($value, "{$var_name}[{$key}]", $separator);
                    } else {
                        $ret .= Ae_Url::array2queryString($value, "{$key}", $separator);
                    }
                } else {
                    if ($var_name) {
    ...
    function getHidden($query = null, $var_name="", $glue = "\n") {
        ...
        if (is_array($query)) {
            foreach ($query as $key => $value) {
                ...
                if (is_array($value)) {
                    if ($var_name) {
                        $ret .= Ae_Url::getHidden($value, "{$var_name}[{$key}]");
                    } else {
                        $ret .= Ae_Url::getHidden($value, "{$key}");
                    }
                } else {
                    if ($var_name) 
    ...
    function toSingleButtonForm($buttonAttribs = array(), $formAttribs = array()) {
        $buttonAttribs = Ae_Util::m(array('type' => 'submit'), $buttonAttribs);
        $formAttribs = Ae_Util::m(array('method' => 'post', 'action' => $this->toString(false)), $formAttribs);
        $res = "<form ".Ae_Util::mkAttribs($formAttribs).">".$this->getHidden()."<input ".Ae_Util::mkAttribs($buttonAttribs)." /></form>";
        return $res;
    }
    ...
    static function guess($withPathInfo = false) {
        ...
        $res = new Ae_Url($protocol[0].'://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']);
        if ($withPathInfo) {
