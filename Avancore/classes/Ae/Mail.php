...
class Ae_Mail {
    ...
    function & factory($templateName, $to, $defaultSubject = false, $from = false, $data = array(), $textTemplate = false) {
        $res = new Ae_Mail($templateName, $to, $defaultSubject, $from, $data, $textTemplate);
        return $res;
    }
    
    function Ae_Mail ($templateName, $to = false, $defaultSubject = false, $from = false, $data = array(), $textTemplate = false) {
    ...
    function fetchTemplate($templateName, $vars = array()) {
        if ($this->templateObject && is_a($this->templateObject, 'Ae_Template')) {
    ...
    function _getAddress(& $entry) {
        ...
        if (is_string($entry)) {
            ...
            $title = Ae_Util::getObjectProperty($entry, 'emailTitle', false);
            $email = Ae_Util::getObjectProperty($entry, 'email', false);
        }
