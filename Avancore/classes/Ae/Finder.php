...
class Ae_Finder extends Ae_Autoparams {
    ...
    protected function doOnInitialize() {
        if (strlen($this->anySortCriterionName)) {
			$this->registerCriterion(new Ae_Finder_Criterion_SortByProperty(array(
				'name' => $this->anySortCriterionName,
			)));
        }
        
        if (strlen($this->anySubstringCriterionName)) {
			$this->registerCriterion(new Ae_Finder_Criterion_SearchByProperties(array(
    ...
    protected function registerCriterion($criterion) {
        if (is_array($criterion)) $criterion = Ae_Autoparams::factory($criterion, 'Ae_Finder_Criterion');
        elseif (!$criterion instanceof Ae_Finder_Criterion) throw new Exception ("\$criterion must be an instance of Ae_Finder_Criterion");
    ...
    function getSqlDb() {
        if ($this->sqlDb === false) {
            $this->sqlDb = new Ae_Sql_Db_Ae(Ae_Dispatcher::getInstance()->database);
        }
        return $this->sqlDb;
    }
     ...
     */
    function createSqlSelect(Ae_Sql_Db $sqlDb = null) {
        ...
        $sqlSelect = new Ae_Sql_Select($sqlDb, $prototype);
        foreach ($this->listCriteria() as $i) {
            $c = $this->getCriterion($i);
     ...
     */
    function createCollection(Ae_Sql_Db $sqlDb = null) {
    ...
    function notifyCriterionChanged(Ae_Finder_Criterion $criterion, $oldValue, $oldEnabled) {
    }
