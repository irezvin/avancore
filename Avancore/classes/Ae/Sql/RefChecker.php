...
define('AE_RC_NUM_OUTGOING', 'numOutgoing');
define('AE_RC_NUM_NO_REF', 'numNoRef');
define('AE_RC_NUM_CORRECT', 'numCorrect');
define('AE_RC_NUM_MISSING', 'numMissing');

define('AE_RC_FIX_DELETE', 'delete');
define('AE_RC_FIX_SET_TO_NON_REF', 'setToNonRef');

class Ae_Sql_RefChecker {
	...
	function Ae_Sql_RefChecker ($options = array()) {
	...
	function _setSchema(& $schema) {
		if (!is_a($schema, 'Ae_Sql_Dbi_Database')) trigger_error("\$schema must be an instance of Ae_Sql_Dbi_Database", E_USER_ERROR);
		$this->_schema = & $schema;
	}
	...
	/**
	 * @param Ae_Sql_Select $selectForRelatedTables
	 * @param $whereKey
	...
	/**
	 * @param Ae_Sql_Select_Table $selTable
	 * @return unknown_type
	 */
	...
	function listStatColumns(& $select, $tableName = false) {
		...
		foreach ($select->listTables() as $i) {
			...
			$c = array(
				AE_RC_NUM_OUTGOING => $this->_db->nameQuoteBody($t->alias."_".AE_RC_NUM_OUTGOING),
				AE_RC_NUM_NO_REF => $this->_db->nameQuoteBody($t->alias."_".AE_RC_NUM_NO_REF),
				AE_RC_NUM_CORRECT => $this->_db->nameQuoteBody($t->alias."_".AE_RC_NUM_CORRECT),
				AE_RC_NUM_MISSING => $this->_db->nameQuoteBody($t->alias."_".AE_RC_NUM_MISSING),
			);
	...
	function _getNonRefCriterion(& $t, $tableName, $primaryAlias) {
		foreach($t->joinsOn as $otherColName => $myColName) {
			if (is_a($myColName, 'Ae_Sql_Expression')) continue;
	...
	function getRelStatColumns(& $select, $asArray = false) {
		...
		foreach ($select->listTables() as $i) {
			...
			$cols = array(
				"SUM($refSrc) AS ".$this->_db->n($t->alias."_".AE_RC_NUM_OUTGOING),
				"SUM(IF($refSrc,0,1)) AS ".$this->_db->n($t->alias."_".AE_RC_NUM_NO_REF),
				"SUM(IF($refDest,0,1)) AS ".$this->_db->n($t->alias."_".AE_RC_NUM_CORRECT),
				"SUM(IF($refDest,1,0))".$this->_db->n($t->alias."_".AE_RC_NUM_MISSING)
			);
			$res[] = $cols;  			
		}
		if (!$asArray || ($asArray === 'flat')) $res = Ae_Util::flattenArray($res);
	...
	function fixRecords($tableName, $otherTableName = false, $relName = false, $action = AE_RC_FIX_DELETE, $getSql = false) {
		$sql = array();
		$select = & $this->createSelect($tableName, 't', $otherTableName, $relName, 'LEFT JOIN', true);
		$primaryAlias = $select->getEffectivePrimaryAlias();
		foreach ($select->listTables() as $i) {
			...
			foreach (array_values($t->joinsOn) as $colName) {
				if (is_string($colName)) {
					...
					if ($action === AE_RC_FIX_SET_TO_NON_REF) {
			...
			} elseif ($action === AE_RC_FIX_DELETE) {
				$sql[] = "DELETE ".($this->_db->n($select->getEffectivePrimaryAlias())).".* FROM ".$select->getFromClause(false)." WHERE ".$select->getWhereClause(false);
			} else {
				trigger_error("Unknown \$action value: '".$action."', valid are AE_RC_FIX_SET_TO_NON_REF, AE_RC_FIX_DELETE", E_USER_ERROR);
			}
		}
