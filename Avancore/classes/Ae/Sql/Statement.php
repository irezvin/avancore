 ...
 * Example: 
 * 
 * $db->query(Ae_Sql_Statement::factory("UPDATE foobar WHERE [[colName]] = {{value}}", array('colName' => 'id', 'value' => 'some unescaped string'));  
 */
class Ae_Sql_Statement extends Ae_Sql_Expression {
    ...
    function & factory($parts, $params = array(), $extraOptions = array()) {
        ...
        if (is_array($extraOptions)) Ae_Util::ms($options, $extraOptions);
        $res = Ae_Util::factoryWithOptions($options, 'Ae_Sql_Statement', 'class', true);
        return $res;
    }
    
    function Ae_Sql_Statement($options = array()) {
    ...
    function _getQParams() {
        ...
        foreach (array_diff(array_keys($this->params), array_keys($this->_qp)) as $k) {
            $n++;
            $v = & $this->params[$k];
            if (is_a($v, 'Ae_Sql_Expression') && method_exists($v, 'applyGlobalParams')) {
                $gParams = $this->params;
    ...
    function _quotePart(& $part, $qParams) {
        if (is_string($part)) {
            ...
            $res = implode(' ', $qParts);
        } elseif (is_a($part, 'Ae_Sql_Expression')) {
            ...
            trigger_error("Unsupported statement part type: ".(is_object($part)? get_class($part) : gettype($part))
                ."; only a string, an array or an Ae_Sql_Expression instance are allowed", E_USER_ERROR);
        }
        return $res;
    }
