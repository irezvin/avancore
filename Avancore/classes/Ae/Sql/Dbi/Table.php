...
class Ae_Sql_Dbi_Table extends Ae_Sql_Dbi_Object {
    ...
    function Ae_Sql_Dbi_Table(& $inspector, $name, & $database, $extras = array()) {
    ...
    function listColumns() {
        if ($this->_columns === false) {
            ...
            foreach ($allColumns as $colName => $colData) {
                $col = new Ae_Sql_Dbi_Column($this->_inspector, $colName, $this, $colData);
    ...
    function listIndices() {
        if ($this->_indices === false) {
            ...
            if (isset($this->_extras['indices']) && is_array($this->_extras['indices'])) {
                Ae_Util::ms($allIndices, $this->_extras['indices']);
            }
            foreach ($allIndices as $name => $data) {
                $obj = new Ae_Sql_Dbi_Index($this->_inspector, $name, $this, $data);
    ...
    function listRelations() {
        if ($this->_relations === false) {
            ...
            foreach ($allRelations as $name => $data) {
                $obj = new Ae_Sql_Dbi_Relation($this->_inspector, $name, $this, $data);
    ...
    function hasOnlyReferenceFields() {
        ...
        foreach ($this->listIncomingRelations() as $relId) {
            ...
            $columns = array_merge($columns, Ae_Util::array_values($rel->columns));
        }
     ...
     * @see Ae_Sql_Dbi_Table::hasOnlyReferenceFields
     * @return array|false  
     */
        ...
        if (is_array($res) && count($res) == 2) {
            ...
            foreach (array_keys($res) as $r) {
                ...
                if ($rel->table == $this->name) { // this is an incoming relation...
                    ...
                    Ae_Util::ms($relFields, array_keys($rel->columns)); 
                }
            }
