...
class Ae_Sql_NestedSets {
    ...
    function Ae_Sql_NestedSets($options = array()) {
        $this->parentIdOfRoot = null;
        Ae_Util::smartBind($options, $this);
    }
    ...
    /**
     * @param Ae_Sql_Blocker $blocker
     */
    function setBlocker(& $blocker) {
        if (!is_a($blocker, 'Ae_Sql_Blocker')) trigger_error("\$blocker must be an instance of Ae_Sql_Blocker", E_USER_ERROR);
    ...
    function & _stmt($parts, $params = array(), $useCache = null) {
        ...
        if ($useCache) {
            ...
            if (!isset($this->_stmtCache[$md])) {
                $this->_stmtCache[$md] = Ae_Sql_Statement::factory($parts, $this->_getCommonParams()); 
            }
            $this->_stmtCache[$md]->applyParams($params);
            $res = $this->_stmtCache[$md];
        } else {
            $res = Ae_Sql_Statement::factory($parts, $this->_getCommonParams($params));
        }
        return $res;
    }
    ...
    function _getCommonParams($extraParams = array()) {
        ...
        if (strlen($this->alias)) {
            ...
            $res['tableName'] = new Ae_Sql_Statement('[[origTableName]] AS [[alias]]');
        } else {
        ...
        if ($this->treeCol && strlen($this->treeId)) {
            $res['tc'] = new Ae_Sql_Statement('AND [[treeCol]] = {{treeId}}');
            $res['origTc'] = new Ae_Sql_Statement('AND [[origTreeCol]] = {{treeId}}');
        } else {
            $res['tc'] = new Ae_Sql_Expression('');
            $res['origTc'] = new Ae_Sql_Expression('');
        }
    ...
    function getNode($idOrData) {
        ...
        $res = $this->_db->fetchRow($this->_stmt(
            ...
            array('crit' => new Ae_Sql_Expression($this->_db->valueCriterion($data)))
        ));
        return $res;
    }
...
//                      }
                        ...
                        $this->query($this->_stmt('
                            ...
                            ', array('parentId' => $parentId, 'leftOrder' => $leftOrder, 'rightOrder' => $rightOrder, 'newOrder' => $order, 'id' => $id, 'delta' => new Ae_Sql_Expression($delta))
                        ));
    ...
    function getChildren ($idOrNode, $withNode = false, $nLevels = false, $orderByLevel = false, $groupById = false) {
        ...
        if ($node) {
            if ($nLevels !== false)
                $maxLevelExpression = Ae_Sql_Statement::factory(
                    ' AND [[levelCol]] <= {{maxLevel}}', 
                    array('maxLevel' => $node['level'] + $nLevels) 
                );
            else
                $maxLevelExpression = new Ae_Sql_Expression(' AND 1');
    ...
    function getChildrenExtended($idOrNode, $extraJoins = false, $extraWhere = false, $orderBy = false, $groupBy = false, $withNode = false, $nLevels = false) {
        ...
        if ($node) {
            ...
            if ($extraJoins !== false) {
                if (is_a($extraJoins, 'Ae_Sql_Expression')) $extraJoinsParam = $extraJoins;
                    else $extraJoinsParam = new Ae_Sql_Expression($extraJoins);
            } else {
                $extraJoinsParam = new Ae_Sql_Expression('');
            }
            
            
            
            if ($orderBy !== false) {
                if (is_a($orderBy, 'Ae_Sql_Expression')) $orderByParam = array('ORDER BY ', $orderBy);
                    else $orderByParam = new Ae_Sql_Expression('ORDER BY '.$orderBy);
            } else {
                $orderByParam = new Ae_Sql_Expression('');
            }
            
            
            
            if ($extraWhere !== false) {
                if (is_a($extraWhere, 'Ae_Sql_Expression')) $extraWhereParam = array('AND ', $extraWhere);
                    else $extraWhereParam = new Ae_Sql_Expression('AND '.$extraWhere);
            } else {
                $extraWhereParam = new Ae_Sql_Expression('');
            }
            
            
            
            if ($groupBy !== false) {
                if (is_a($groupBy, 'Ae_Sql_Expression')) $groupByParam = array('GROUP BY ', $groupBy);
                    else $groupByParam = new Ae_Sql_Expression('GROUP BY '.$groupBy);
            } else {
                $groupByParam = new Ae_Sql_Expression('');
            }
    ...
    protected function query($stmt) {
        Ae_Callbacks::getInstance()->call(self::debugStmtCallback, $this, $stmt, self::debugBeforeQuery);
        $res = $this->_db->query($stmt);
        Ae_Callbacks::getInstance()->call(self::debugStmtCallback, $this, $stmt, self::debugAfterQuery);
        return $res;
    }
