...
class Ae_Legacy_Controller_Std_Admin_Template extends Ae_Template_Html {
	...
	/**
	 * @var Ae_Legacy_Controller_Response_Http
	 */
...
<?php 	    
	...
	function _showWrapper($content) {
        $this->htmlResponse->addCssLib('{AECSS}/managers.css', true);
?>
...
<?php 	    
	...
	function showManager() {
        ...
        if ($this->managerResponse && isset($this->managerResponse->hasToRedirect) && ($redir = $this->managerResponse->hasToRedirect)) {
            $bu = $ctx->getUrl();
            $u = new Ae_Url($redir);
            $bu->query = Ae_Util::m($allState, $u->query, true);
...
?>
    		<div class='avantravel'>
    			<form <?php echo Ae_Util::mkAttribs($formAttribs); ?>>
