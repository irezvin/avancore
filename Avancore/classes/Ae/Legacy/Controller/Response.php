 ...
 */
class Ae_Legacy_Controller_Response {
    ...
    /**
     * @var array of Ae_Legacy_Controller_Response_Part
     */
    var $parts = array();
    ...
    function copyFrom(Ae_Legacy_Controller_Response $otherResponse) {
    ...
    function fromArray(array $data) {
        $this->clear();
        $vars = array_intersect(array_keys($data), array_keys(Ae_Util::getClassVars(get_class($this))));
    ...
    function getData($path, $default = null) {
        return Ae_Util::getArrayByPath($this->data, Ae_Util::pathToArray($path), $default);
    }
    
    /**
     * Finds part with specified signature and appends its placeholder into response content.
     * 
     * $partSignature can be either a string - a class name; an array - an Ae_Legacy_Controller_Response_Part prototype; or a ready
     * Ae_Legacy_Controller_Response_Part instance itself.
     * 
     * @param $data Part data
     * @param string|array|Ae_Legacy_Controller_Response_Part $partSignature
     ...
     */
    function createPlaceholder($data, $partSignature = 'Ae_Legacy_Controller_Response_Part', $appendToContent = false) {
        $p = $this->findPart($partSignature);
        if (!$p) {
            $p = Ae_Legacy_Controller_Response_Part::factory($partSignature);
            $this->parts[] = $p;
        }
    ...
    /**
     * @param string|array|Ae_Legacy_Controller_Response_Part $partSignature
     * @return Ae_Legacy_Controller_Response_Part
     */
