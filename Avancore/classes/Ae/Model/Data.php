...
/**
 * TODO Consider moving all unified accessors into separate class (i.e. Ae_Model_Data_Accessor). Since during several requests they won't be used at all (it's not hard to imagine (c) Samsung)
 * TODO Another step: consider moving all mutation-related methods (bind, check.., set.., delete..) into other separate class (i.e. Ae_Model_Data_Mutator)
 * TODO If we will build web requests based on Ae_Model_Data, mutators will be used on each request. It's useful, it's unified... how fast it should be? 
 */
...
class Ae_Model_Data {
    ...
    function bind($src, $ignore = false) {
        ...
        if ($ignore)
            foreach ($ignore as $ignorePath) if (strlen($ignorePath)) {
                Ae_Util::unsetArrayByPath($src, Ae_Util::pathToArray($ignorePath));
            }
    ...
    function getError() {
        if ($errors = $this->getErrors()) return Ae_Util::implode_r(";\n", $errors);
        else return false;
    }
        ...
        if ($this->_errors) {
            if ($propertyName) {
                ...
                if ($concat === false) $res = $this->_errors;
                    ...
                    $res = Ae_Util::implode_r($concat, $this->_errors);
                }
            }
        } else $res = false;
        return $res;
    ...
    function _checkOwnFields() {
        ...
        foreach ($this->listOwnFields() as $propName) {
            if ($this->_getPlural($propName) && $keys = $this->listProperty($propName)) {
                $fieldsToCheck = array_merge($fieldsToCheck, Ae_Util::concatManyPaths($propName, $keys));
                ...
                if ($pi->plural) {
                    foreach ($this->listProperty($propName) as $key) {
                        if ($pi->loadToCheck || $this->isAssocLoaded($propName, $key)) {
                            ...
                            if (is_a($assocObject, 'Ae_Model_Data')) {
    ...
    function _mergeErrors($extraErrors, $propName = false, $key = false) {
        if (is_array($extraErrors)) {
            $prefix = Ae_Util::concatPaths($propName, $key);
            foreach ($extraErrors as $path => $errors) {
                $destPath = Ae_Util::concatPaths($prefix, $path);
                Ae_Util::setArrayByPath($this->_errors, Ae_Util::pathToArray($destPath), $errors, array(& $this, '_setErrorByPathCallback'));
            }
        }
    }
    ...
    function getField($propName, $key = false) {
        ...
        list($head, $tail) = Ae_Util::pathHeadTail($propName);
        ...
        if ($assocClass = $this->_getAssocClass($head)) { 
            ...
            if ($plural = $this->_getPlural($head)) {
                if ($tail && $key === false) {
                    list($tail, $key) = Ae_Util::pathBodyTail($tail);
                }
    ...
    function _getAssociatedField($head, $tail, $key = false) {
        ...
        if ($bga === true) {
            ...
            if ($plural = $this->_getPlural($head)) {
                list($subKey, $tail) = Ae_Util::pathHeadTail($tail);
    ...
    function hasAssoc ($propName, $key = false) {
        ...
        if ($assocClass && $tail) {
            if (($plural = $this->_getPlural($head)) && !strlen($key)) {
                list ($newTail, $newKey) = Ae_Util::pathBodyTail($tail);
    ...
    function setField($propName, $value, $key = false) {
        ...
        if ($assocClass) {
            ...
            if (!strlen($key)) list($tail, $key) = Ae_Util::pathBodyTail($tail);
    ...
    function getPropertyInfo($propName, $onlyStatic = false) {
        ...
        $res = new Ae_Model_Property($this, $propName, $onlyStatic, $arrPropInfo);
        $this->doAfterGetPropertyInfo($res);
        return $res; 
    }
    
    function hasProperty($propName) {
        return ($this->_getStaticPropertyInfoArr(Ae_Util::pathToArray($propName), false, false) !== false);
    }
    ...
    function _matchPaths($searchPaths, $arrPath) {
        ...
        foreach ($searchPaths as $testPath) {
            $arr = Ae_Util::pathToArray($testPath);
            if (count($arr) == $c) {
                $match = true;
                for ($i = 0; $i < $c; $i++) 
    ...
    function isFieldEmpty(& $formOptions, $value = false) {
        if (is_a($formOptions, 'Ae_Model_Property')) {
            $fo = $formOptions->toFormOptions();
        } elseif(is_array($formOptions)) {
            $fo = $formOptions;
        } else {
            trigger_error ("Wrong formOptions format, Ae_Model_Property or array must be specified", E_USER_ERROR);
        }
        if (func_num_args() < 2) {
