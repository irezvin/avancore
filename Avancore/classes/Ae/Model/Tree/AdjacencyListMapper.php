...
class Ae_Tree_AdjacencyListMapper extends Ae_Model_Mapper implements Ae_I_Tree_Mapper_AdjacencyList {
    ...
    function getNodeClass() {
        return 'Ae_Tree_AdjacencyListImpl';
    }
    
    function loadOriginalDataForNode(Ae_Tree_AdjacencyListImpl $node) {
        $nid = $node->getNodeId();
    ...
    function loadNodes(array $ids) {
        ...
        foreach ($this->loadOriginalData($ids) as $id => $node) {
            ...
            $objNode = new Ae_Tree_AdjacencyListImpl($prot);
            $res[$id] = $objNode;
        }
        return $res;
    }
