...
class Ae_Tree_NestedSetsDebug {
    ...
    function setWriter(Ae_Debug_Log_AbstractWriter $writer = null) {
        $this->writer = $writer;
    }
     ...
     */
    function getWriter() {
        return Ae_Debug_Log_AbstractWriter;
    }
    ...
    function canDebug($item) {
        $res = false;
        if ($item instanceof Ae_Model_Tree_NestedSetsImpl) {
    ...
    function implCallback($stage, $impl, $res = null) {
        if ($this->canDebug($impl)) {
            if ($stage == Ae_Model_Tree_NestedSetsImpl::debugBeginSelfStore || $stage == Ae_Model_Tree_NestedSetsImpl::debugBeginSelfDelete) {
                $this->beginCollectStatements();
                $this->buffer['_oldProblems'] = $this->getProblems($impl);
                $db = $impl->getNestedSets()->getDb();
                //$db->query("START TRANSACTION");
            } elseif ($stage == Ae_Model_Tree_NestedSetsImpl::debugEndSelfStore || $stage == Ae_Model_Tree_NestedSetsImpl::debugEndSelfDelete) {
                $oldProb = $this->buffer['_oldProblems'];
                $newProb = $this->getProblems($impl);
                unset($this->buffer['_oldProblems']);
                if ($newProb && $newProb != $oldProb) {
                    $this->log("NS error was caused by transaction {$stage} in ", $impl->getContainer()->getDataFields(), Ae_Debug_Log::getInstance()->getTrace(false, array('Ae_Debug_Log', get_class($this), 'Ae_Callbacks')));
    ...
    function nsCallback($ns, $stmt, $stage) {
        if ($stage === Ae_Sql_NestedSets::debugAfterQuery && is_array($this->buffer)) {
            $this->buffer[] = ''.$stmt;
        }
    }
    
    function registerCallbacks() {
        Ae_Callbacks::getInstance()->addHandler(Ae_Model_Tree_NestedSetsImpl::debugCallback, array($this, 'implCallback'));
        Ae_Callbacks::getInstance()->addHandler(Ae_Sql_NestedSets::debugStmtCallback, array($this, 'nsCallback'));
    }
