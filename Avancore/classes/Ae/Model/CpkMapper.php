 ...
 */
class Ae_Model_CpkMapper extends Ae_Model_Mapper {
    ...
    function Ae_Model_CpkMapper($tableName, $recordClass, $pk) {
    ...
    function putToArrayByPk(& $record, & $dest) {
        Ae_Util::simpleSetArrayByPath($dest, $record->getPrimaryKey(), $record, true);
    }
    ...
    function getFromArrayByPk(& $src, $pk, $default = null) {
        $res = & Ae_Util::simpleGetArrayByPath($src, $pk, $default);
        return $res;        
    }
    ...
    function getFlatArrayWithPks(& $src) {
        $res = array();
        Ae_Model_CpkMapper::_iterateRecursive($res, $src, array(), count($this->pk));
        return $res;
    }
    ...
    function _iterateRecursive(& $res, & $src, $prefix, $height) {
        ...
        if ($height <= 0) {
            ...
            foreach(array_keys($src) as $k) {
                $partialKey = $prefix;
                $partialKey[] = $k;
                Ae_Model_CpkMapper::_iterateRecursive($res, $src[$k], $partialKey, $height);
            }
        }
    }
