...
class Ae_Application_Adapter extends Ae_Autoparams {
    ...
    protected function setConfigFiles($value) {
        if ($value !== false) $value = Ae_Util::toArray($value);
    ...
    protected function intGetConfigFromFiles() {
        ...
        foreach ($this->getConfigFiles() as $fn) {
            $config = array();
            require($fn);
            Ae_Util::ms($this->configFromFiles, $config);
        }
    ...
    protected function intGetOverrides() {
        if ($this->overrides === false) {
            ...
            Ae_Util::ms($this->overrides, $this->configFromInit);
            Ae_Util::ms($this->overrides, $this->getConfigFromFile());
        }
    }
    ...
    protected function guessUrls() {
        ...
        if (!isset($this->config['webAssetsUrl']) || !isset($this->config['adminImagesUrl'])) {
            if (!isset($u) && isset($this->config['webUrl'])) $u = new Ae_Url($this->config['webUrl']);
            if (isset($u)) {
                $u->query = array();
    ...
    protected function intGetArrConfig() {
        if ($this->config === false) {
            Ae_Util::ms($this->config, $this->intGetConfigFromFiles());
            Ae_Util::ms($this->config, $this->configFromInit);
            ...
            if (isset($this->config['overrides']) && is_array($this->config['overrides'])) {
                ...
                Ae_Util::ms($this->config, $this->config['overrides']);
            }
        }
        return $this->config;
    }
    ...
    function getAssetPlaceholders() {
        $res = Ae_Util::toArray($this->intGetConfigValue(substr(__FUNCTION__, 3)));
        return $res;
    }
    ...
    function getAppInitOptions() {
        return Ae_Util::toArray($this->intGetConfigValue(substr(__FUNCTION__, 3)));
    }
