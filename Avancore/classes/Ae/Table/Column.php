...
class Ae_Table_Column {
    
    /**
     * Link to parent table Ae_Table object that this column belongs to
     * @var Ae_Table
     * @access private
     */
    var $_table = false;
    ...
    /**
     * Whether get{$fieldName} funtion call is preferred over Ae_Model_Data::getFoo()
     * @var bool|string
     */
    var $useGetter = false;
    ...
    */
    ...
    /**
     * @param object table Parent Ae_Table instance
     * @param string name of the column
     * @param array settings
     */
    function Ae_Table_Column(& $table, $name, $settings = array()) {
        $this->_table = & $table;
        $this->_name = $name;
    ...
    /**
     * @return array Array representation of the cell TD tag's attributes (for Ae_Util::mkAttribs function)  
     */
    ...
    function updateAttribs() {
        $this->_cellAttribs = Ae_Util::mkAttribs($this->getCellAttribs()); 
    }
    
    /**
     * @return array Array representation of the header TH tag's attributes (for Ae_Util::mkAttribs function)  
     */
    ...
    function getRecordProperty(& $record, $fieldName) {
        ...
        if ($this->decorator) $res = Ae_Decorator::decorate($this->decorator, $res, $this->decorator);
        
        return $res;
    }

    function determineGetter(& $record, $fn) {
        if (is_a($record, 'Ae_Model_Data') && $this->useAeDataFacilities && $this->_staticMeta) return array('getWithGetField', null);
     ...
     * @param Ae_Model_Data $record
     */
    function getPropertyInfo(& $record, $fieldName = false, $isStatic = false) {
        if (is_a($record, 'Ae_Model_Data') && $this->useAeDataFacilities) {
    ...
    function showHeader($rowCount, $rowNo = 1) {
        ...
        if (!$this->hidden) echo "<th title='{$this->_name}' ".Ae_Util::mkAttribs($this->getHeaderAttribs($rowCount, $rowNo)).">".$this->getTitle()."</th>";
    }
