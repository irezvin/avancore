 ...
 */
class Ae_Form_Control_List extends Ae_Form_Control_Listable {
    ...
    /**
     * @param Ae_Model_Values $provider
     */
    function setValuesProvider(& $provider) {
        if ($provider && !is_a($provider, 'Ae_Model_Values')) trigger_error ("\$provider should be instance of Ae_Model_Values", E_USER_ERROR);
        $this->_valuesProviderPrototype = false;
        $this->_valuesProvider = false;
        $this->_valuesGetter = false;
        if ($provider) {
