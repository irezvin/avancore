...
class Ae_Param_Condition_Number extends Ae_Param_Condition {
    ...
    function getTranslations() {
        return array_merge(parent::getTranslations(), Ae_Autoparams::getObjectProperty($this, array('type', 'lt', 'le', 'gt', 'ge', 'nz', 'ne')));
    }
    ...
    function match($value, & $errors = array(), Ae_I_Param $param = null) {
        ...
        if (!is_numeric($value)) $this->regError($errors, 'non_numerc', $px, '{param} must be numeric', $param);
            ...
            if (($this->ne !== false) && in_array($value, $a = Ae_Util::toArray($a)))
