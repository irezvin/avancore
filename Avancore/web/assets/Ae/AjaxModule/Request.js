/**
 * Requires: YAHOO, Ae_WithOptions, Ae_Util
 */

Ae_AjaxModule_Request = function(options) {
    this.id = ++Ae_AjaxModule_Request.instanceId;
    this.data = {};
    this.rqOptions = {};
    this.callbackArg = {};
...
Ae_AjaxModule_Request.prototype = {
        ...
        _getUrl: function(action, params) {
            ...
            if (action && this.actionParamName) {
                ...
                Ae_Util.override(p, params);
            } else {
                p = params;
            }
            if (p) res += (res.indexOf('?') >= 0? '&' : '?') + Ae_Util.makeQuery(p, '', true);
            return res;
        },
...
Ae_AjaxModule_Request.instanceId = 0;

Ae_Util.extend(Ae_AjaxModule_Request, Ae_WithOptions);