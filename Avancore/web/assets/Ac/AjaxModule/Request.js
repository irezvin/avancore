/**
 * Requires: YAHOO, Ac_WithOptions, Ac_Util
 */

Ac_AjaxModule_Request = function(options) {
    this.id = ++Ac_AjaxModule_Request.instanceId;
    this.data = {};
    this.rqOptions = {};
    this.callbackArg = {};
...
Ac_AjaxModule_Request.prototype = {
        ...
        _getUrl: function(action, params) {
            ...
            if (action && this.actionParamName) {
                ...
                Ac_Util.override(p, params);
            } else {
                p = params;
            }
            if (p) res += (res.indexOf('?') >= 0? '&' : '?') + Ac_Util.makeQuery(p, '', true);
            return res;
        },
...
Ac_AjaxModule_Request.instanceId = 0;

Ac_Util.extend(Ac_AjaxModule_Request, Ac_WithOptions);