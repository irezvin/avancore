...
class Ac_Form_Control_Template_Placeholders extends Ac_Form_Control_Template_Basic {
    ...
    var $defaultWrapperPart = 'divWrapper';
    ...
    function renderChildContainer(Ac_Form_Control $child, & $controlsById, $part = null) {
        if ($part === null) $part = $this->defaultWrapperPart;
        $cid = $child->name;
        $control = $child->fetchPresentation(true, $part? false : null);
        if ($child->showWrapper && strlen($part)) {
            $control = $this->fetch($part, array($child, $control));
        }
