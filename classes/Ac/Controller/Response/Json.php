...
class Ac_Controller_Response_Json extends Ac_Controller_Response_Html {
	...
	function setInnerResponse(Ac_Controller_Response $innerResponse, $add = false) {
	...
	function getJson($asArray = false) {
	    ...
	    foreach ($this->innerResponses as $innerResponse) {
	        ...
	        if ($innerResponse instanceof Ac_Controller_Response_Json && isset($arr['values'])) {
	            $tmp = $arr['values'];
	            unset($arr['values']);
			...
			if (isset($res['jsonComments']) && strlen($res['jsonComments'])) {
                if (!Ac_Controller_Response_Json::$skipJsonComments) 
                    $jc .= "\n" .$res['jsonComments'];
			} 
        ...
        if (!$asArray) {
            ...
            if (!Ac_Controller_Response_Json::$skipJsonComments && strlen($jc)) {
                $jc = str_replace("/*", "", $jc);
                $jc = str_replace("*/", "", $jc);
                $allRes .= "\n\n/*\n".$jc."\n*/";
            }
    ...
    function mergeWithResponse ($subResponse, $withTitleAndPathway = true, $putContent = false) {
        ...
        if ($subResponse instanceof Ac_Controller_Response_Json) {
