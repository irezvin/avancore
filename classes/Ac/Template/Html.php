...
class Ac_Template_Html extends Ac_Template {
    ...
    function addPageTitle($title) {
        if (is_a($this->htmlResponse, 'Ac_Controller_Response_Html')) $this->htmlResponse->addPageTitle($title);
    }
    ...
    function addJsLib($jsLib, $isLocal = true, $atBeginning = false) {
        if (is_array($jsLib)) {
            ...
            if (is_a($this->htmlResponse, 'Ac_Controller_Response_Html')) {
                $this->htmlResponse->addJsLib($jsLib, $isLocal, $atBeginning);
            }
            else {
                echo Ac_Controller_Response_Html::getJsScriptTag($jsLib, $isLocal);
            }
        }
    }
    ...
    function addCssLib($cssLib, $isLocal = true) {
        if (is_array($cssLib)) {
            ...
            if (is_a($this->htmlResponse, 'Ac_Controller_Response_Html')) $this->htmlResponse->addCssLib($cssLib, $isLocal);
            else {
                echo Ac_Controller_Response_Html::getCssLibTag($cssLib, $isLocal);
            }
        }
    }
    ...
    function addMeta($metaName, $metaValue) {
        if (is_a($this->htmlResponse, 'Ac_Controller_Response_Html')) $this->htmlResponse->addMeta($metaName, $metaValue);
    }
    
    function appendPathway($url, $text) {
        if (is_a($this->htmlResponse, 'Ac_Controller_Response_Html')) $this->htmlResponse->appendPathway($url, $text);
    }
    ...
    function getHtmlHelper () {
        $res = $this->getTemplateHelper ('Ac_Template_Helper_Html');
        return $res; 
    }
