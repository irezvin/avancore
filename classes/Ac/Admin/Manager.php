...
class Ac_Admin_Manager extends Ac_Controller {
     ...
     */
    function getForm() {
        if ($this->_form !== false) return $this->_form;
        if (!$this->getRecord()) return null;
...
/**
    ...
    function listFeatures() {
        if ($this->_features === false) $this->initFeatures();
    ...
    function getFeature($f) {
        ...
        $res = $this->_features[$f];
        return $res;
    }
    
    /**
     * Returns list of the available actions
     * @param bool $currentGroupOnly List actions for form/list, regarding $this->isForm() / $this->isList()
     */
    
    function listActions ($currentGroupOnly = false) {
        if ($this->_actions === false) $this->initActions();
        
        if (!$currentGroupOnly) return array_keys($this->_actions);
        
        $res = array();
        foreach ($this->_actions as $k => $action) {
    ...
    protected function initActions() {
        ...
        foreach ($actionPrototypes as $ak => $p) {
            if (!isset($p['id']) || !$p['id']) $p['id'] = $ak;
            $action = Ac_Prototyped::factory($p, 'Ac_Admin_Action');
            if (!strlen($action->id)) $action->id = ''.$ak;
    ...
    function listProcessings() {
        if ($this->_processings === false) $this->initProcessings();
        return array_keys($this->_processings);
    }
    
    protected function initProcessings() {
        ...
        foreach ($this->listActions() as $a) {
            $action = $this->getAction($a);
            $proto = $action->getProcessingPrototype();
            if (!$proto) continue;
            if (isset($this->_processings[$a])) {
                trigger_error("\$processingPrototype of action '{$a}' conflicts with processing '{$i}'", E_USER_WARNING);
                continue;
            }
            $this->_processings[$a] = $proto;
        }
    }
    
    function listSubManagers() {
        if ($this->_subManagers === false) $this->initSubManagers();
        return array_keys($this->_subManagers);
    }
    
    protected function initSubManagers() {
