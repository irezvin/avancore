...
*** TODO Ac_Admin_Manager: 'datalink' restrictions are not always applied to sub-manager processing [2014-12-09] :admin:bug:minor:
...
*** TODO Ac_Admin_Manager: when action returns "raw" (non-wrapped, no-html output), output is still wrapped in outer manager's details [2014-12-09] :admin:bug:minor:
