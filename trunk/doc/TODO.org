...
*** TODO Ac_Model_Object::reset() fix [2014-07-25 Fri] 	       :model:cg:bug:
    - [ ] Ac_Model_Mapper::EVENT_ON_GET_DEFAULTS
      - [ ] is called once
      - [ ] cached result is cleared if addEventLister(EVENT_ON_GET_DEFAULTS)
    - [ ] Ac_Model_Mapper::getRelationDefaults() - returns fields necessary for relations' implementation
      - [ ] generated mapper has pre-filled array
      - [ ] for non-generated mapper relations are asked
      - [ ] relations are asked too for
	- [ ] generated mapper with altered relations
	- [ ] for mapper with defined handlers for EVENT_ON_GET_RELATION_PROTOTYPES
	- [ ] for mapper that created any relations after triggering EVENT_ON_RELATION_NOT_FOUND
    - [ ] reset(), doOnReset() also call mixables
    - [ ] reset() un-registers record from mappers' $recordsCollection
...
*** TODO reduce getNNIds() to free field only [2014-08-09 Sat] :model:
    see [[freeFields]]
*** TODO fix Ac_Model_Relation::countSrc/Dest [2014-08-17 Sun] :model:bug:
