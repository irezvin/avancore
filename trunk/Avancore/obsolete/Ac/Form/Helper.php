...
//        var_dump($name, $options);
    ...
    function doShowTextInput($name, $value, $attribs, $options) {
        ...
        $attribs['type'] = 'text';
        echo '<input '.Ac_Util::mkAttribs($attribs).' />';
    }
    ...
    function doShowTextArea($name, $value, $attribs, $cols, $rows, $options) {
        ...
        echo '<textarea '.Ac_Util::mkAttribs($attribs).' >'.$value.'</textarea>';
    }
...
//        var_dump("$width $height $cols $rows ");
    ...
    function doShowDateInput($name, $value, $attribs, $options) {
        ...
        if (isset($options['imageAttribs'])) {
            ...
            echo '<input type="image" '.Ac_Util::mkAttribs($imageAttribs).' />';
        } else {
    ...
    function doShowSelectList($name, $value, $valueList, $multiple, $dummyCaption, $useInputs, $attribs, $options) {
        ...
        if ($useInputs) {
            ...
            if ($dummyCaption !== false) {
                ...
                echo '<div><input '.Ac_Util::mkAttribs($itemAttribs).'/>&nbsp;'.$dummyCaption.'</div>';
            }
            foreach ($valueList as $v => $t) {
                $itemAttribs = $attribs;
                $itemAttribs['value'] = $v;
                $itemAttribs['checked'] = (((string)$v == (string)$value));
                echo '<div><input '.Ac_Util::mkAttribs($itemAttribs).'/>&nbsp;'.$t.'</div>';
            }
        } else {
            $attribs['multiple'] = $multiple;
            echo '<select '.Ac_Util::mkAttribs($attribs).'>';
            if ($dummyCaption !== false) {
                $itemAttribs = $attribs;
                $itemAttribs['value'] = '';
                if (!strlen($value)) $itemAttribs['checked'] = true;
                echo '<option '.Ac_Util::mkAttribs($itemAttribs).'>'.$dummyCaption.'</option>';                
            }
            foreach ($valueList as $v => $t) {
                $itemAttribs['value'] = $v;
                $itemAttribs['selected'] = (((string)$v == (string)$value));
                echo '<option '.Ac_Util::mkAttribs($itemAttribs).'>'.$t.'</option>';                
            }
            echo '</select>';
        }
    }
    ...
    function doShowWrapper($name, $options, $content, $showCaption, $caption, $showRequired, $required, $showErrors, $error) {
        ...
        echo $content;
        if (is_array($error)) $error = Ac_Util::implode_r("\n", $error);
    ...
    function formatDateTime($value, $options) {
        $res = $value;
        if (strlen($res)) {
            if (isset($options['outputDateFormat'])) $res = Ac_Util::date($res, $options['outputDateFormat']);
        }
        return $res;
    }
