 ...
 * @package Avancore
 * @subpackage Params
 * @example param/source.php Ae_Param_Source usage example.
 */
abstract class Ae_Param_Source {
    ...
    /**
     * Parent source.
     * @var Ae_Param_Source|false|null
     * @access private
     */
    ...
    /**
     * Array that contains list of child Ae_Param_Source instances.
     * It is used to remove references to this instance (to free memory on destroy() call).
     * @see Ae_Param_Source::destroy()
     * @var array
     * @access private
     */
     ...
     * 
     * @param Ae_Param_Source|null $source Reference to the parent parameter source.
     * @param string $key Key of this source in the parent.
     */
    function setParent(Ae_Param_Source $source = null, $path = false) {
        $this->parent = $source;
        $this->path = $path;
    }
     ...
     */
    function getRoot() {
        $res = Ae_Param_Impl_Paths::getRoot($this, 'getParent');
        return $res;
    }
    
    /**
     * Returns path of this source in the sources hierarchy (relative to root).
     * @see Ae_Param_Impl_Paths::getPath
     * @param bool $asString
     * @return string
     */
    function getPath($asString = false) {
        $res = & Ae_Param_Impl_Paths::getPath($this, $asString, 'key', '_parent');
        return $res;
    }
     ...
     * @param bool $mustBeValue True if result should *not* be Ae_Param_Source and if source is found it will be consifered as *not found*.   
    ...
    function getByPathIfFound($path, & $result, $mustBeValue = true) {
        ...
        $src = & Ae_Param_Impl_Paths::getByPath($this, $path, '_parent', 'getSource', 'isSource');
        $res = false;
        if ($src) {
            if ($src->hasKey($last)) {
                if ($mustBeValue) {
    ...
    function destroy($recursive) {
        ...
        foreach (array_keys($this->_sourceInstances) as $k) {
            ...
            if (Ae_Util::sameObject($this->_sourceInstances[$k]->_parent, $this)) {
     ...
     */
    function Ae_Param_Source($options = array()) {
