...
class Ae_Controller_Response_Json extends Ae_Controller_Response_Html {
	...
	function setInnerResponse(Ae_Controller_Response $innerResponse, $add = false) {
	...
	function getJson($asArray = false) {
	    ...
	    foreach ($this->innerResponses as $innerResponse) {
	        ...
	        if ($innerResponse instanceof Ae_Controller_Response_Json && isset($arr['values'])) {
	            $tmp = $arr['values'];
	            unset($arr['values']);
