...
define('CALLBACK_AE_DATABASE_AFTER_QUERY', 'Ae_Database.afterQuery');
...
class Ae_Database {
    ...
    function Ae_Database($options = array()) {
        if (get_class($this) == 'Ae_Database') trigger_error ("Attempt to instantiate abstract class Ae_Database", E_USER_ERROR);
        if (isset($options['config']) && is_a($options['config'], 'Ae_Config')) {
            $this->_config = & $options['config']; 
        } else {
            $disp = & Ae_Dispatcher::getInstance();
            $this->_config = & $disp->config;
        }
        //$this->_tmpDir = $disp->getDir().'/cache';
        //$this->_tmpDir = $disp->getCacheDir();
