...
class Ae_Upload_Manager {
    
    var $_uploadsLifetime = 3600;
    
    var $_uploadsCacheDir = false;
    
    var $_uploadClass = 'Ae_Upload_File';
    ...
    /**
     * @var Ae_Upload_Storage_Abstract
     */
    var $_storage = false;
    
    var $storageOptions = array('class' => 'Ae_Upload_Storage');
    ...
    function Ae_Upload_Manager ($options = array()) {
    ...
    function getUpload($id, $load = false) {
        $res = false;
//        Ae_Debug_FirePHP::getInstance()->log($id, 'getUpload()');
...
//                Ae_Debug_FirePHP::getInstance()->log($fn, 'loading internal data');
                $res = & $this->factory(array('internalData' => $internalData));
                $this->_cacheUpload($res);
            }
//            Ae_Debug_FirePHP::getInstance()->log($fn, 'getUpload()');
        } else {
//            Ae_Debug_FirePHP::getInstance()->log('id invalid', 'getUpload()');
        }
        return $res;
    }
    ...
    /**
     * @return Ae_Upload_Storage_Abstract
     */
    function getStorage() {
        if ($this->_storage === false) {
            if (is_array($this->storageOptions)) $this->_storage = & Ae_Util::factoryWithOptions($this->storageOptions, 'Ae_Upload_Storage', 'class', false);
        }
        return $this->_storage;
    }
    
    /**
     * @param Ae_Upload_Storage_Abstract $storage
     */
    ...
    function & factory ($options = array()) {
        ...
        $res = Ae_Util::factoryWithOptions($options, $this->_uploadClass);
        return $res;
    }
