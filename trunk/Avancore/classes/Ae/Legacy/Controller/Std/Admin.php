...
class Ae_Legacy_Controller_Std_Admin extends Ae_Legacy_Controller_Std_Web {
	
	var $_templateClass = 'Ae_Legacy_Controller_Std_Admin_Template';
	...
	function applyState(Ae_Legacy_Controller_Context_Http $c) {
        $p = $c->getDataPath(true);
        if (strlen($sv = $this->getStateVarName())) {
            if (!isset($_SESSION[$sv])) $_SESSION[$sv] = array();
            $state = Ae_Util::getArrayByPath($_SESSION[$sv], $p);
            ...
            Ae_Util::unsetArrayByPath($state, array('keys'));
            
            $data = $c->getData();
            if (isset($data['action']) && $data['action'] == 'list') {
	            if (!is_array($state)) $state = array();
	            if (!count($data)) $state = array();
	            Ae_Util::ms($state, $c->getData());            
	            $c->setData($state);
	            Ae_Util::setArrayByPath($_SESSION[$sv], $p, $state);
            }
        }
	}
	...
	function executeManager() {
	    ...
	    if (in_array($mapperId, $mapperClasses)) {
            ...
            $context = new Ae_Legacy_Controller_Context_Http($contextOptions);
            $context->populate('request', $px = $mapperId);
            
            $this->applyState($context);
            
            $managerConfig = array('mapperClass' => $mapperId);
            $manager = new Ae_Admin_Manager($context, $managerConfig, $px);
