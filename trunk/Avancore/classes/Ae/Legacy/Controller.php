...
class Ae_Legacy_Controller {
     ...
     * @access protected 
     */
    var $_defaultResponseClass = 'Ae_Legacy_Controller_Response_Html';
    ...
    /**
     * @var Ae_Legacy_Controller_Context_Info 
     */
    var $_contextInfo = false;
    
    /**
     * @var Ae_Legacy_Controller_Response
     */
    var $_response = false;
    ...
    /**
     * @param Ae_Legacy_Controller_Context $context
     */
    function __construct ($context = null, $options = array(), $instanceId = false) {

        // Make the call Ae_Autoparams::factory compatible 
    	...
    	if (!(is_object($context) && $context instanceof Ae_Legacy_Controller_Context)) {
        	if ($context instanceof Ae_Url) $url = $context->cloneObject();
            elseif (is_string($context)) $url = new Ae_Url($context);
            else $url = Ae_Url::guess(true);
            
            $context = null;
            
            $path = '';
            if (is_array($context)) $path = $context;
                elseif (is_string($context)) $path = Ae_Util::pathToArray($context);
            $context = new Ae_Legacy_Controller_Context_Http();
    ...
    function bindFromRequest() {
        if ($this->_bound === false) {
            ...
            $this->_rqWithState = Ae_Util::m($this->_state, $this->_rqData, true);
    ...
    /**
     * @return Ae_Legacy_Controller_Context_Info
     */
    ...
    function getUrl($extraParams = array()) {
        $res = false;
        if (is_a($this->_context, 'Ae_Legacy_Controller_Context_Http')) {
            ...
            $extraParams = Ae_Util::m($myParams, $extraParams);
    ...
    function _getTplData() {
        $res = Ae_Util::ms($this->templateExtraVars, $this->_tplData);
        $res['controller'] = & $this;
