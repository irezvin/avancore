...
class Ae_Param extends Ae_Autoparams implements Ae_I_Param_WithSource {
    
    /**
     * @var Ae_I_Param_Source
     */
    protected $source = null;
    
    /**
     * @var Ae_I_Param_Destination
     */
    ...
    static function applyConditionsAndFilters($value, array $conditions, array $filters, & $ok, Ae_I_Param $param = null, array & $errors = array(), 
        $evaluateAllConditions = false) {
            ...
            for ($i = 0; ($i < $c) && ($ok || $evaluateAllConditions); $i++) {
                ...
                if (strlen($propName)) $propVal = $param? Ae_Autoparams::getObjectProperty($param, $cond[$c]->getPropName()) : null;  
                    else $propVal = $value;
                $ok = $cond[$i]->match($propVal, $e, $param);
                if ($e) {
                    $tmp = array($k[$i] => $e);
                    Ae_Util::ms($errors, $tmp);
                }
            }
    ...
    function setPath($path) {
        if (!is_array($path) && $path !== false) $path = Ae_Util::toArray($path);
        $this->path = $path;
    }
    ...
    function setDestination(Ae_I_Param_Destination $destination = null) {
    ...
    /**
	 * @return Ae_Param_Parent
     */
    function getParent() {
        return $this->parent;
    }
    
    function setParent(Ae_Param_Parent $parent = null) {
    ...
    function getIdPath() {
        $parents = Ae_Autoparams::getAllParents($this, 'parent', true, true);
        $res = implode('.', Ae_Autoparams::getObjectProperty($parents, 'id'));
        return $res;
    }
