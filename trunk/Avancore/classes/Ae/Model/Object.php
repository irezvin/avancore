 ...
 */
define('AE_CHANGES_BEFORE_SAVE', 1);

/**
 * If $model::tracksChanges() returns true or AE_CHANGES_AFTER_SAVE, old values will be available in afterSave() method 
 */
define('AE_CHANGES_AFTER_SAVE', 2);

class Ae_Model_Object extends Ae_Model_Data {
    ...
    function _checkOwnFields() {
        ...
        if ($dbp) {
            ...
            foreach ($dbp as $indexName => $pks) {
                ...
                $fn = current($ff);
                $px = count($ff) > 1? AE_SUCH_VALUES_OF_FIELD_MULTIPLE : AE_SUCH_VALUES_OF_FIELD_SINGLE;
                $this->_errors[$fn]['index'][$indexName] = sprintf(AE_RECORD_BY_INDEX_ALREADY_EXISTS, $px, $this->_describeIndex($indexName, $ff));
            }
        }
    }
    ...
    function bind($array, $ignore = '') {
        ...
        if (!$this->_hasOldBehaviour()) {
            ...
            if (!is_array( $array )) {
                ...
                return Ae_Util::bindArrayToObject( $array, $this, $ignore );
            }
        }
...
*/
    ...
    /**
     * Rely on own bind() and check() methods against Ae_Model_Data ones
     */
    ...
    */
     ...
     *
     * @param Ae_Model_Object $otherObject
    ...
    /**
     * Returns true if both associations are 'equal' (refer to same number of objects for which Ae_Model_Object::equals() returns true), false otherwise
     * 
     * @param Ae_Model_Object $otherObject
     * @param string $assocName Name of association property
     * @param array|false subAssocList List and order of associations to compare in associated objects (is passed as second parameter to Ae_Model_Object::equals() call). False means 'use default'
     */
     ...
     * This method will be called only when standard implementation (Ae_Model_Object::equals) already had successfully compared 
    ...
    function cleanupReferences(& $otherObject) {
        foreach (array_keys(get_object_vars($this)) as $k) {
            if (is_object($this->$k) && Ae_Util::sameObject($this->$k, $otherObject)) {
                $this->$k = null;
            } elseif (is_array($this->$k)) {
                $tmp = & $this->$k;
                foreach (array_keys($tmp) as $kk) {
                    if (is_object($tmp[$kk]) && Ae_Util::sameObject($tmp[$kk], $otherObject)) {
                        unset($tmp[$kk]);
                    }
                }
                unset($tmp);
    ...
    function cleanupMembers() {
        ...
        foreach (get_class_vars(get_class($this)) as $k => $v) if (isset($this->$k)) {
            if (is_array($this->$k)) {
                $tmp = & $this->$k;
                foreach (array_keys($tmp) as $kk) {
                    if (is_a($tmp[$kk], 'Ae_Model_Object')) {
                        $o = & $tmp[$kk];
            ...
            if (is_a($this->$k, 'Ae_Model_Object')) {
                $o = & $this->$k;
                $o->cleanupReferences($this);
                unset($o);
            }
