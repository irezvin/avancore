/**
 * Requires: Ac_Util, Ac_WithOptions
 */

if (!window.Ac_Util) throw "Ac_AjaxModule requires Ac_Util namespace; window.Ac_Util not found";

Ac_AjaxModule = function (options) {
    ...
    this.instanceId = ++Ac_AjaxModule.instanceId;
 ...
 */
Ac_AjaxModule.Forward = {
};

Ac_AjaxModule.instanceId = 0;


Ac_AjaxModule.prototype = {
    ...
    getAllForms: function(dialogId) {
        var res = [];
        for (var i in this.dialogs) if (Ac_Util.hasOwnProperty(this.dialogs, i)) {
    ...
    handleFormSubmit: function(event, form, extraData) {
        ...
        if (pos >= 0 && (action.slice(pos, pos + hrefPrefix.length) == hrefPrefix)) {
            ...
            if (args[0] && args[0].length && args[0].slice(0, 1) === '&') {
                ...
                queryArgs = Ac_Util.parseQuery(args.join('.'));
            }
            var data = Citex.Util.getData(form, form._submitButton || null);
            Ac_Util.override(queryArgs, data);
            if (extraData && ((typeof extraData) == 'object')) Ac_Util.override(queryArgs, extraData);
            data = queryArgs;
    ...
    callScripts: function(data, args) {
        return Ac_Util.callScripts(data, args, this);
    },
    ...
    executeCommand: function(command, extraRequestOptions, extraQueryArgs, target) {
        ...
        if (extraQueryArgs) Ac_Util.override(queryArgs, extraQueryArgs);
        if (c) {
            var m = 'execute' + Ac_Util.ucFirst(c);
            if (this[m] && typeof(this[m]) == 'function') {
                ...
                /**
                 *  Client-hosted method can return Ac_AjaxModule.Forward to force the request;
                 *  it can alter args and options before that too. 
                 */ 
                if (res === Ac_AjaxModule.Forward) {
                    if (!queryArgs) {
                        queryArgs = {cmd: args};
                    }
    ...
    _doRequest: function(action, data, extraRequestOptions, isPost, target) {
        ...
        if (this.requestOptions) Ac_Util.override(options, this.requestOptions);
        ...
        Ac_Util.override(options, extraRequestOptions);
        
        var request = new Ac_AjaxModule_Request(options);
    ...
    getDialog: function(id) {
        ...
        if (!this.dialogs[id]) {
            var defOptions = {constructorFunc: Ac_AjaxModule_YuiPanelDialog};
            
            if (typeof this.defaultDialogOptions === 'object') 
                Ac_Util.override(defOptions, this.defaultDialogOptions);
            
            if (typeof this.dialogOptions[id] === 'object')
                Ac_Util.override(defOptions, this.dialogOptions[id]);
    ...
    clearDialogs: function() {
        for (var i in this.dialogs) if (Ac_Util.hasOwnProperty(this.dialogs, i)) this.dialogs[i].destroy();
        this.dialogs = {};
    }
    
}

Ac_Util.extend(Ac_AjaxModule, Ac_WithOptions);